...
```{r}
datos1 <- datos %>% 
  select(Campo, Metodo, Estudio_caso, Perspectiva_gral, triple_hélice: ed_superior_final, motivacion: Otros., `Publication Year`) %>% 
...
```{r, fig.width=7}
datos1 %>% 
  select(Campo, Metodo, Estudio_caso, Perspectiva_gral, ) %>% 
...
```{r}
plot_bar(datos, by = "Perspectiva_gral", ggtheme = theme_light(), nrow = 4L,ncol = 2L)
```
...
Interpretación pendiente, pero por ejemplo el cuadrante inferior izquierdo indica que las variables k, c, e, ad, af y p están positivamente relacionadas entre sí, formando un cluster de variables. Lo mismo en el derecho las variables h, f, aa, b. Lo mismo con i, w, v x, y, ab. Lo mmismo b y j. Se trata entonces, desde el punto de vista interpretativo, de buscar e in tentar explicar clusters.

Por otro lado, mirando espacialmente el gráfico vemos que la variables w y h ,por ejemplo, están negativamente asociadas, ya que están en sectores opuestos del plano. Las variables que están más cerca del centroide tienen menos importancia porque en esos caso las correlaciones son menores.
...
Por otro lado, la interpretación debe considerar las dos dimensiones principales, la horizontal y la vertical. La horizontal opone a las variables k, e, i, c versus las variables h, aa, f, b  y j. Esa es la dimensión 1. ¿Le podemos poner un nombre a esa dimensión o componente horizontal?. En caso de que así... bingo, hemos hecho un descubrimiento teórico.

Lo mismo respecto de la dimensión vertical, si se le puede encontrar un sentido teórico ya se recontra justifica un artículo.
...
La dimensión o componente 1 es la dimensión horizontal del gráfico, la dimensión o componente 2 es la vertical. Esta tabla interactiva ayuda mucho a la interpretación del gráfico de componentes principales.
